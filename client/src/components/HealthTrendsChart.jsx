...
function PremiumTooltip({ active, payload, vitalMap, t }) {
  ...
  return (
    <div className="rounded-2xl border border-white/40 bg-white/75 p-3 shadow-2xl backdrop-blur-xl">
      <p className="mb-2 text-xs font-black uppercase tracking-[0.16em] text-zinc-500">
        {dateLabel ? formatTooltipDate(dateLabel) : t("Trend") || "Trend"}
      </p>
  ...
  return (
    <div className="rounded-2xl border border-dashed border-white/50 bg-white/40 p-8 text-center backdrop-blur-sm">
      ...
      <p className="text-base font-black text-zinc-800">
        {t("Need more data for trends") || "Need more data for trends"}
      </p>
...
export default function HealthTrendsChart({ records = [], vitalTypes = [] }) {
  ...
  const chartData = useMemo(
    () => formatRecordsForTrends(records, vitalTypes),
    [records, vitalTypes],
  );
  const [visibleTypes, setVisibleTypes] = useState(
    () => new Set(vitalTypes.map((vital) => vital.type)),
  );
      ...
      <Card className="overflow-hidden border border-white/20 bg-gradient-to-br from-blue-100/45 to-teal-100/30 p-6 backdrop-blur-xl">
        <div className="mb-4 flex items-center gap-2">
          ...
          <p className="text-lg font-black text-zinc-900">
            {t("Vital Trends Dashboard") || "Vital Trends Dashboard"}
          </p>
        </div>
        <EmptyTrendsState t={t} />
      </Card>
    );
  }
    ...
    <Card className="overflow-hidden border border-white/20 bg-gradient-to-br from-blue-100/45 to-teal-100/30 p-6 backdrop-blur-xl">
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        ...
        <p className="rounded-full border border-white/60 bg-white/70 px-3 py-1 text-[10px] font-black uppercase tracking-[0.16em] text-zinc-500">
          {t("Click a vital to focus") || "Click a vital to focus"}
        </p>
      </div>
      ...
      <div className="h-[330px] rounded-2xl border border-white/45 bg-white/35 p-3 backdrop-blur-xl">
        <ResponsiveContainer width="100%" height="100%">
          {/* ... inside your LineChart component ... */}
          <LineChart
            data={chartData}
            margin={{ top: 10, right: 10, left: 10, bottom: 10 }} // Provides space for the glow
            style={{ overflow: "visible" }} // Prevents the drop-shadow from clipping
          >
            <XAxis
              dataKey="date"
            ...
            />
            <Tooltip
              content={<PremiumTooltip vitalMap={vitalMap} t={t} />}
            ...
            {activeVitals.map((vital) => (
              <Line
                ...
                connectNulls
                // FIXED: Moved filter into the style object
                style={{
                  filter: `drop-shadow(0 0 8px ${vital.stroke}cc) drop-shadow(0 4px 6px ${vital.stroke}33)`,
                }}
